function Analyze =  VAD_comparing_28_04_15_(audifile,addnoice,addnoice1,addnoice2,addnoice3)
...
%}
[~,vad12] = stat1VAD_f_27_04_15(audifile,addnoice,1,2,0.97);
X12 = vad12;
[~,vad12] = stat1VAD_f_27_04_15(audifile,addnoice,1,2,0.97);
X121 = vad12;
%L_F_SqwenrgyVAD = sum( abs(X12 - X121))
%numberVoiceFrameSqwenrgyVAD = sum(X12)
alg(1).name = 'stat1_dd_VAD';
...
Analyze = alg;
Y2 = stat1VAD_f_27_04_15(audifile,addnoice,1,2,0.97);
...
plot(stat1VAD_f_27_04_15(audifile,addnoice1,1,2,0.97),'k'),hold on
plot(weakfricdetVADe(audifile,addnoice1).*1.2,'m'),hold on
...
plot(stat1VAD_f_27_04_15(audifile,addnoice2,1,2,0.97),'k'),hold on,title('Fig13. X + 2*N')
plot(stat1VADe(audifile,addnoice2).*1.6,'r'),hold on,legend('stat1_dd_VAD')
plot(weakfricdetVADe(audifile,addnoice2).*1.2,'m'),hold on
...
plot(stat1VAD_f_27_04_15(audifile,addnoice3,1,2,0.97).*0.9,'k'),hold on
plot(stat1VADe(audifile,addnoice3).*1.6,'r'), legend('stat1VAD'),hold on
title('Fig.14 X+3*N')
plot(weakfricdetVADe(audifile,addnoice3).*1.2,'m'),hold on
